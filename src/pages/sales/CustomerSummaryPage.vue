...
const totals = computed(() => ({
  ...
  netExposure: rows.value.reduce((sum, row) => sum + Number(row.net_customer_exposure ?? row.balance ?? 0), 0),
}))
    ...
    <div class="flex h-full min-h-0 min-w-0 flex-col gap-2 rounded-b-2xl rounded-tr-2xl border border-slate-200 bg-white p-2.5 shadow-sm lg:p-3">
      <header class="flex min-w-0 flex-none flex-wrap items-center justify-between gap-2">
        <div class="min-w-0">
          ...
          <h1 class="truncate text-base font-black leading-5 text-slate-950">AR Customer Summary</h1>
        </div>
      ...
      <div class="grid min-w-0 flex-none items-center gap-2 border-b border-slate-100 pb-2 md:grid-cols-[minmax(0,1fr)_auto]">
        <label class="min-w-0">
          ...
          <input v-model="search" class="h-9 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none transition focus:border-[#24a1db] focus:ring-2 focus:ring-[#e9f6fb]" placeholder="Search customer" />
        </label>
        <div class="workspace-table-scroll flex min-w-0 flex-nowrap items-center gap-2 overflow-x-auto md:justify-end">
          ...
          <span class="inline-flex h-9 shrink-0 items-center rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-extrabold text-slate-600">
            Official AR: {{ formatMoney(totals.balance) }}
          </span>
          <span class="inline-flex h-9 shrink-0 items-center rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-extrabold text-slate-600">
      ...
      <div v-if="loading" class="p-4 text-sm font-bold text-slate-500">Loading customer summary...</div>
        ...
        <table class="min-w-full divide-y divide-slate-100 text-sm">
          <thead class="bg-slate-50 text-left text-xs font-black uppercase tracking-wide text-slate-400">
            <tr>
              ...
              <th class="px-3 py-2 text-right">Official AR</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="row in filteredRows" :key="row.customer_id" class="hover:bg-slate-50">
              ...
              <td class="px-3 py-2 text-right font-black">{{ formatMoney(row.official_ar_balance ?? row.balance) }}</td>
              <td class="px-3 py-2 text-right font-bold text-slate-700">{{ formatMoney(row.unapplied_deposit_total ?? 0) }}</td>
              <td class="px-3 py-2 text-right font-black">{{ formatMoney(row.net_customer_exposure ?? row.balance) }}</td>
